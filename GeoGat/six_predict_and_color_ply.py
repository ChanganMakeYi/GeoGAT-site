...
def interpolate_color_4(score):
    """方案4: 橙色->黄色->白色"""
    score = np.clip(score, 0.0, 1.0)
    if score <= 0.5:
        t = score / 0.5
        r = 255
        g = int(165 + 90 * t)  # 从橙(255,165,0)到黄(255,255,0)
        b = 0
    else:
        t = (score - 0.5) / 0.5
        r = 255
        g = 255
        b = int(255 * t)  # 从黄到白(255,255,255)
    return r, g, b


def interpolate_color_5(score):
    """方案5: 灰色->蓝色->青色"""
    score = np.clip(score, 0.0, 1.0)
    if score <= 0.5:
        t = score / 0.5
        r = int(128 * (1 - t))
        g = int(128 * (1 - t))
        b = int(128 * (1 - t) + 255 * t)  # 从灰(128,128,128)到蓝(0,0,255)
    else:
        t = (score - 0.5) / 0.5
        r = 0
        g = int(255 * t)
        b = 255  # 从蓝到青(0,255,255)
    return r, g, b


def interpolate_color_6(score):
    """方案6: 红色->紫色->黑色"""
    score = np.clip(score, 0.0, 1.0)
    if score <= 0.5:
        t = score / 0.5
        r = 255
        g = 0
        b = int(255 * t)  # 从红(255,0,0)到紫(255,0,255)
    else:
        t = (score - 0.5) / 0.5
        r = int(255 * (1 - t))
        g = 0
        b = int(255 * (1 - t))  # 从紫到黑(0,0,0)
    return r, g, b


# 配色方案字典
COLOR_SCHEMES = {
    1: interpolate_color_1,
    2: interpolate_color_2,
    3: interpolate_color_3,
    4: interpolate_color_4,
    5: interpolate_color_5,
    6: interpolate_color_6,
}
...
def update_ply_with_colors(ply_file_path, output_ply_file, vertex_scores, center_indices, predictions, color_func):
    """更新 PLY 文件，添加顶点颜色和面预测分数，使用指定的配色函数"""
    ...
    for i, line in enumerate(vertex_lines):
        ...
        if len(parts) < 9:
            ...
            continue
        vertex_data = parts[:9]
        r, g, b = color_func(vertex_scores[i])
...
def main():
    ...
    parser.add_argument("--scheme", type=int, default=1, choices=[1, 2, 3, 4, 5, 6], help="配色方案选择 (1-6)")
    ...
    color_func = COLOR_SCHEMES[args.scheme]
    print(f"使用配色方案: {args.scheme}")

    try:
        update_ply_with_colors(args.ply_file, args.output_ply, vertex_scores, center_indices, predictions, color_func)
    except Exception as e:
